...
class EnhancedMetaActivityTrackerWithAirtable:
    """
    Daily Meta activity tracker:
    - Human activities only
    - Complete hierarchy: Campaign → AdSet → Ad
    - Airtable mapping
    - Smart fetch (based on last Timestamp)
    - Safe window: min 6h, max 72h
    - Append + dedupe into Google Sheets
    """
    ...
    # Smart fetch safety limits
    MIN_FETCH_HOURS = 6      # never fetch less than this
    MAX_FETCH_HOURS = 72     # never fetch more than this daily
    ...
    def _find_best_brand_match(self, brand_name):
        """Find best matching brand from Airtable using simple fuzzy logic"""
        if not brand_name:
            return None
    ...
    def get_account_activities(self, ad_account_id, hours=24):
        """Get activities for an account from last N hours (human-only)"""
    ...
    def _build_complete_hierarchy(self, activity):
        ...
        try:
            ...
            # CASE 1: Campaign Group (Campaign)
            if object_type == 'campaign_group':
                ...
                if campaign_data:
                    ...
                    hierarchy['Campaign_Status'] = campaign_data.get(
                        'effective_status', campaign_data.get('status', 'N/A')
                    )
            ...
            # CASE 2: Campaign (AdSet)
                ...
                if adset_data:
                    ...
                    hierarchy['AdSet_Status'] = adset_data.get(
                        'effective_status', adset_data.get('status', 'N/A')
                    )
            ...
            # CASE 3: AdGroup (Ad)
                ...
                if ad_data:
                    ...
                    hierarchy['Ad_Status'] = ad_data.get(
                        'effective_status', ad_data.get('status', 'N/A')
                    )
    ...
    def _process_account(self, account, hours=24):
        ...
        for activity in activities:
            ...
            if timestamp:
                try:
                    dt = datetime.strptime(
                        timestamp.replace('Z', '+00:00').split('+')[0],
                        '%Y-%m-%dT%H:%M:%S'
                    )
            ...
            results.append({
                ...
                'Action': activity.get('translated_event_type',
                                       activity.get('event_type', 'Unknown')),
    ...
    def fetch_meta_activities(self, hours=24):
        ...
        for obj_type, count in sorted(
                self.debug_stats['object_types_found'].items(),
                key=lambda x: x[1],
                reverse=True):
    ...
    def map_airtable_to_activities(self, activities_df):
        ...
        brand_col = next((col for col in possible_brand_cols
                          if col in self.brand_mapping_df.columns), None)
        fb_manager_col = next((col for col in possible_fb_manager_cols
                               if col in self.brand_mapping_df.columns), None)
        brand_manager_col = next((col for col in possible_brand_manager_cols
                                  if col in self.brand_mapping_df.columns), None)
        team_col = next((col for col in possible_team_cols
                         if col in self.brand_mapping_df.columns), None)
            ...
            if brand_name:
                ...
                if normalized:
                    self.brand_mapping_dict[normalized] = {
                        ...
                        'FB_Manager': row[fb_manager_col] if fb_manager_col and pd.notna(row[fb_manager_col])
                        else 'Not Assigned',
                        'Brand_Manager': row[brand_manager_col] if brand_manager_col and pd.notna(row[brand_manager_col])
                        else 'Not Assigned',
                        'Current_Team': row[team_col] if team_col and pd.notna(row[team_col])
                        else 'Not Assigned'
                    }
        ...
        # Fetch run timestamp (when we pulled from Meta)
    ...
    def upload_to_sheets(self, df, sheet_name='Meta_Activities_Log', append_mode=False):
        ...
        - If append_mode=True: read old data, dedupe by _unique_id, merge, and write back.
        """
        if self.gspread_client is None:
    ...
    # ==========================
    # MAIN RUN (DAILY TRACKER)
    # ==========================

    def run(self, hours=12, append_mode=True, save_csv=False):
        """
        Main execution pipeline for DAILY TRACKER.
        - Smart fetch based on last Timestamp in Meta_Activities_Log
        - Only human activities
        - Hours window capped between MIN_FETCH_HOURS and MAX_FETCH_HOURS
        - Append + dedupe upload
        """
        start_time = time.time()

        print("=" * 80)
        print("🗂️ ENHANCED META ACTIVITY TRACKER - COMPLETE HIERARCHY (DAILY)")
        ...
        if append_mode:
            ...
            if last_entry_time:
                ...
                raw_adjusted = int(hours_since_last) + 2  # small buffer

                # Apply min/max caps
                if raw_adjusted < self.MIN_FETCH_HOURS:
                    adjusted_hours = self.MIN_FETCH_HOURS
                elif raw_adjusted > self.MAX_FETCH_HOURS:
                    adjusted_hours = self.MAX_FETCH_HOURS
                else:
                    adjusted_hours = raw_adjusted
                ...
                print(f"   Raw adjusted window:    {raw_adjusted}h")
                print(f"   Final fetch window:     {adjusted_hours}h "
                      f"(min={self.MIN_FETCH_HOURS}, max={self.MAX_FETCH_HOURS})")
                print("=" * 80 + "\n")

                if hours_since_last > self.MAX_FETCH_HOURS:
                    print("⚠️ Gap is larger than MAX_FETCH_HOURS.")
                    print("   → Fetching only the last "
                          f"{self.MAX_FETCH_HOURS}h. Older gap will need manual backfill if required.")
                ...
                self.log_github_activity(
                    ...
                    f'(cap {self.MIN_FETCH_HOURS}-{self.MAX_FETCH_HOURS}h)'
                )
            else:
                # No previous data - first daily run
                hours = max(hours, self.MIN_FETCH_HOURS)
                print(f"\nℹ️ No previous data found, using default: {hours} hours\n")
                self.log_github_activity('📋 First Run', f'Fetching last {hours} hours')

        self.log_github_activity(
            '🚀 Tracker Started',
            f'Fetching last {hours} hours with complete hierarchy (human-only)'
        )

        # Fetch Airtable mapping
        ...
        print(f"✅ Campaign data: {campaign_populated}/{len(final_df)} "
              f"({campaign_populated / len(final_df) * 100:.1f}%)")
        print(f"✅ AdSet data:   {adset_populated}/{len(final_df)} "
              f"({adset_populated / len(final_df) * 100:.1f}%)")
        print(f"✅ Ad data:      {ad_populated}/{len(final_df)} "
              f"({ad_populated / len(final_df) * 100:.1f}%)")
        ...
        for level in ['ADSET', 'AD']:
            ...
            if not sample.empty:
                ...
                if level in ['ADSET', 'AD']:
                    ...
                    print(f"     └─ Targeting: {row.get('Gender_Targeting', 'N/A')}, "
                          f"{row.get('Age_Targeting', 'N/A')}")
                if level == 'AD':
        ...
        if save_csv:
            ...
            csv_file = f"meta_activities_COMPLETE_HIERARCHY_DAILY_{timestamp}.csv"
            final_df.to_csv(csv_file, index=False)
            print(f"\n💾 CSV SAVED: {csv_file}")
            print(f"   Total rows: {len(final_df)}")
            print(f"   Total columns: {len(final_df.columns)}")

        # Upload to Sheets in append + dedupe mode
        if self.gspread_client:
            self.upload_to_sheets(final_df, append_mode=append_mode)
            duration = (time.time() - start_time) / 60
            self.log_github_activity(
                '✅ Tracker Completed',
                f'{len(final_df)} activities with complete hierarchy in {duration:.1f}min. '
                f'Range: {time_range}'
            )

        print("\n" + "=" * 80)
        print("✅ COMPLETE - Daily tracker finished successfully!")
        print("=" * 80)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("🚀 META ACTIVITY TRACKER - DAILY GITHUB ACTIONS (ENHANCED)")
    print("=" * 80)
    ...
    # Default CLI hours is just a starting hint; smart fetch will override in append_mode
    hours = 12
    if len(sys.argv) > 1:
        try:
